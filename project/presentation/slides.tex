...
\begin{frame}{Key Terminology}
  ...
  \onslide<+->{%
    \begin{block}{Covariate Shift}
      ...
      \begin{center}
      ${\pDist(\X,\y) = \blue{\pDist(\X)}\green{\pDist(\y \vert \X)}}$ \textbf{\&} ${\pDist\red{'}(\X,\y) = \blue{\pDist}\red{'}\blue{(\X)}\green{\pDist(\y \vert \X)}}$
      \end{center}

      resp.\ \blue{differ in marginals} (${\pDist(\X)}$ vs.\ ${\pDist'(\X)}$) but have \green{identical posterior}~${\pDist(\y \vert \X)}$.
    \end{block}
  }
  \vfill
  \onslide<+->{%
...
\begin{frame}{Risk Estimation}
  ...
  \vfill
  \onslide<+->{\textbf{Restating our Original Goal}: Minimize the expected \textit{test} loss}
  \begin{itemize}[<+->]
    \setlength{\itemsep}{7pt}
    \item \red{\textbf{Problem}}: Test joint distribution is unknown
    \item \green{\textbf{Solution}}: Use an \textit{empirical estimate} as a surrogate
  \end{itemize}
  \onslide<+->{%
...
\begin{frame}{Approaches to Negative Covariate Shift for Doc.\ Classification}
  ...
  \begin{enumerate}[<+->][a]
    ...
    \item \textbf{\blue{Positive\-/Unlabeled Learning}}: Train classifier using $\ptrain$ and~$\utrain$~\cite{Fei:2015}
      ...
      \end{itemize}
    \item \textbf{\blue{Positive\-/Unlabeled Biased Negative Learner}}: Train using $\ptrain$, $\bntrain$ and~$\utrain$
      \begin{itemize}
...
\begin{frame}{Experimental Setup Overview}
  ...
  \begin{itemize}[<+->]
    ...
    \item $\bntrain$~covers only \red{subset} of ${\pDist(\X \vert \y = \ncls)}$'s support but otherwise marginals are \green{unbiased}
    \item (\blue{Reverse}) \onslide<+->{ $\bntrain$~covers ${\pDist(\X \vert \y = \ncls)}$'s \green{complete} support but with marginal \red{bias}}
  \end{itemize}
  \vfill
...
\begin{frame}{Experiments -- Effect of Bias}
  \begin{minipage}[t]{0.69\linewidth}
    ...
    \end{table}
    \vfill
    \onslide<9->{ $\red{\dagger}$ \textbf{Unbiased Marginal}: ${\sbrack{48.2\%, 12.1\%, 39.7\%}}$}
  \end{minipage}
